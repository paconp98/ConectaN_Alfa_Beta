    ...
    public int jugada(Grid tablero, int conecta) {
        ...
        for (int i = 0; i < tablero.getFilas(); i++) {
            ...
            for (int j = 0; j < tablero.getColumnas(); j++) {
                if (tablero.getButton(i, j) != ConectaN.VACIO) {
                    if (tablero.getButton(i, j) == ConectaN.JUGADOR2) {
                        ganarHorz++;
                    } else {
                        ganarHorz = 0;
                    }
        ...
        for (int j = tablero.getColumnas() - 1; j >= 0; j--) {
            if (!tablero.fullColumn(j)) {
                if (tablero.topColumn(j) != ConectaN.JUGADOR2) {
                    ganarHorz++;
                } else {
                    ganarHorz = 0;
                }
        ...
        for (int i = 0; i < tablero.getFilas(); i++) {
            if (tablero.getButton(i, posicion) != ConectaN.VACIO) {
                if (tablero.getButton(i, posicion) != ConectaN.JUGADOR2) {
                    ganarVert++;
                } else {
                    ganarVert = 0;
                }
        ...
        //Pintar Ficha
        return tablero.checkWin(tablero.setButton(posicion, ConectaN.JUGADOR2), posicion, conecta);
    } // jugada          
